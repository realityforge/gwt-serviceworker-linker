...
### [v#{ENV['PRODUCT_VERSION']}](https://github.com/realityforge/gwt-serviceworker-linker/tree/v#{ENV['PRODUCT_VERSION']}) (#{ENV['RELEASE_DATE']}) · [Full Changelog](https://github.com/realityforge/gwt-serviceworker-linker/compare/#{from}...v#{ENV['PRODUCT_VERSION']})
HEADER
...
HEADER
    ...
    stage('GithubRelease', 'Create a Release on GitHub') do
      ...
      client.login
      client.create_release('realityforge/gwt-serviceworker-linker', tag, :name => tag, :body => changes, :draft => false, :prerelease => prerelease)

      candidates = client.list_milestones('realityforge/gwt-serviceworker-linker').select {|m| m[:title].to_s == tag}
      unless candidates.empty?
        milestone = candidates[0]
        unless milestone[:state] == 'closed'
          client.update_milestone('realityforge/gwt-serviceworker-linker', milestone[:number], :state => 'closed')
        end
      end
    end
  end
